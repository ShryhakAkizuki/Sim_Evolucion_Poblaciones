...
#include <stdexcept>
#include "data_structures/Double_Link_Node.hpp"

template<typename T>
class Double_Linked_List : private Double_Link_Node<T> {
private:
    // ----- Atributos -----
    uint32_t _size = 0;
    std::unique_ptr<Double_Link_Node<T>> _head = nullptr;
    std::unique_ptr<Double_Link_Node<T>> _tail = nullptr;
...
public:
    ...
    explicit Double_Linked_List(const &T data){
        Double_Link_Node<T> new_node = std::make_unique<Double_Link_Node<T>>(data);
